...
\newcommand{\YRachievement}{
	...
	The handheld controller that has been designed at Yamamoto's lab in 2016 and 2017 uses voice coil motors (VCMs) for the feedback%TODO cite paper
	. The disadvantage of these motors is that a constant voltage has to be applied to maintain a constant force. This leads to an energy-inefficient system and produces a lot of extra heat. Furthermore, the motors were rated at $13.6$ V with $2$ N/A. In the current implementation, the output force of each palm stimulator was exactly $2$ N.\\
	...
	The goal of this work is to replace the VCM by a set of series elastic actuators (SEAs). Different SEA-based controller designs shall be prototyped and tested on a real robot (topy). Furthermore, various feedback laws shall be implemented and tested to find the most intuitive feedback for the state of the robot.
	
	\section{Design and Analysis}
	\subsection{Series Elastic Actuators}
	%TODO The advantage of the SEA is that the elastic spring element in the system reduces the forces control to a distance control (the compression of the springs). Additionally, the elastic element is shock absorbent and stable for different environments. This means that the controller can be stable even when used with different grip stiffness, for example. \\
