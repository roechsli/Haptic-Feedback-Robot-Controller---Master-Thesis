...
for filename in os.listdir(directory):
    ...
    for i in range(len(df.iloc[:,3]) -3):
        ...
        # reference from wave generator:
        df.iloc[i + 3, 2] = sensor2dist(-float(df.iloc[i + 3, 2])/5 * 1024, PHOTO_MIN_LEFT, PHOTO_MAX_LEFT) 
    ...
    plt.plot(time_vec, photoreceptor_vec)  # plot photoreceptor
    ...
    plt.subplot(2, 1, 2)
    plt.plot(time_vec, motor_volt_vec)  # plot motor voltage
    #plt.legend(['Reference', 'Sensor left'])
    plt.xlabel('Time [s]')
